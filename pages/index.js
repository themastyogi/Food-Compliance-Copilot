...
// ===== App Shell & helpers (top-level, stable) =====
const AppHeader = ({
  user,
  onOpenDownloads,
  onOpenAdmin,
...
function LoginView(props) {
  const {
    ...
    handleSignup, handleLogin, submitForgotPassword,
  } = props;
...
function UpgradeView({ user, isLoading, processUpgrade, setCurrentView }) {
  return (
...
function AdminView({
  user, userMenuOpen, setUserMenuOpen,
  handleLogout, handleUpgradeRequest, setCurrentView,
  userDatabase, updateUserRole, getRoleColor, getRoleIcon
}) {
  return (
...
function DownloadsView({
  user, userMenuOpen, setUserMenuOpen,
  handleLogout, handleUpgradeRequest, setCurrentView,
  doQuickPrompt, getRoleColor, getRoleIcon
}) {
  const links = [
...
function ChatView({
  user, userMenuOpen, setUserMenuOpen,
  handleLogout, handleUpgradeRequest, setCurrentView,
  chatMessages, menuMode, handleSelectTemplate, setMenuMode,
  templateContentHTML, downloadContent,
  isLoading, inputMessage, setInputMessage, sendMessage,
  getRoleColor, getRoleIcon, getUserLimits
}) {
        ...
        {menuMode && (
          <div className="bg-white/10 backdrop-blur-sm text-gray-100 border border-white/20 rounded-lg p-4">
            ...
            <div className="pt-3 flex gap-2">
              ...
              <button className="bg-gray-500/30 hover:bg-gray-500/40 px-3 py-2 rounded" onClick={() => { setMenuMode(false); }}>00) Main menu</button>
            </div>
          </div>
        )}
...
const App = () => {
  ...
  const [userDatabase, setUserDatabase] = useState([]); // admin
  ...
  // Fetch users when entering Admin
  useEffect(() => {
  ...
  const handleSignup = async () => {
    ...
    try {
      const response = await fetch('/api/auth/signup', {
        ...
        body: JSON.stringify({ name: signupName.trim(), email: signupEmail.toLowerCase(), password: signupPassword }),
        credentials: 'include'
      });
  ...
  const submitForgotPassword = async () => {
    ...
    try {
      const response = await fetch('/api/auth/reset-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: forgotEmail.toLowerCase(), newPassword: forgotNewPassword })
      });
      const data = await response.json();
      if (response.ok) {
